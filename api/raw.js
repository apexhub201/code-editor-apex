...
// Valid auth keys from environment variables
...
// In-memory cache for scripts (short-lived optimization, not primary storage)
const scriptCache = new Map();
const CACHE_TTL = 5 * 60 * 1000; // 5 minutes

// ============================================================
// SECURITY GUARD (Integrated)
// ============================================================
...
async function banIP(ip, durationMs = 300000, reason = 'security_violation') {
    try {
        ...
        console.log(`[GUARD] Banned IP: ${ip} - Reason: ${reason}`);
    } catch (error) {
...
async function checkRateLimit(ip, endpoint, maxRequests = 30, windowMs = 60000) {
    ...
    try {
        ...
        // Fail open to avoid blocking legitimate traffic during errors
...
/**
 * Main security guard for raw endpoint
 */
async function securityGuard(req, res, options = {}) {
    ...
    if (options.rateLimit !== false) {
        ...
        if (!rateResult.allowed) {
            return { 
                blocked: true, 
                status: 429, 
                body: { error: 'RATE_LIMITED', retryAfter: Math.ceil((options.windowMs || 60000) / 1000) } 
            };
        }
    }
    ...
    if (options.botDetection !== false) {
        ...
        if (botResult.isBot && botResult.score >= 40) {
            // Log but don't block immediately on first offense
            console.log(`[GUARD] Bot detected: ${ip} - Score: ${botResult.score} - ${botResult.reasons.join(', ')}`);
        }
        if (botResult.score >= 60) {
            ...
            return { 
                blocked: true, 
                status: 403, 
                body: { error: 'ACCESS_DENIED', code: 'SUSPICIOUS_ACTIVITY' } 
            };
        }
    }
    ...
    if (options.requireSession) {
        ...
        if (!session) {
            return { 
                blocked: true, 
                status: 401, 
                body: { error: 'SESSION_REQUIRED', code: 'INVALID_OR_EXPIRED_SESSION' } 
            };
        }
    }
...
async function handleGet(req, res) {
    ...
    // Check for valid auth key (admin bypass)
    ...
    // Admin access with valid key - bypass security
    if (hasValidKey) {
    ...
    // Apply security guard for normal access
    ...
    // Challenge verification (alternative to session for some clients)
    if (challenge && answer) {
        try {
            ...
            console.error('[RAW] Challenge verification error:', error.message);
        }
    ...
    // Check if client is an executor
    ...
    // Generate new challenge for unknown clients
...
async function handleCreate(req, res) {
    try {
        ...
        // Apply light rate limiting for creation
        const guardResult = await securityGuard(req, res, {
        ...
        console.log(`[APEX] Applying Phantom obfuscation for: ${fullName}`);
